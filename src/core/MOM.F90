...
!         call pass_vector(u, v, G%Domain)
        ...
        call cpu_clock_begin(id_clock_diabatic)
        call adiabatic(h, CS%tv, fluxes, dtdia, CS%visc%MLD, G, GV, CS%diabatic_CSp)
        fluxes%fluxes_used = .true.
        call cpu_clock_end(id_clock_diabatic)
...
!         call pass_vector(u, v, G%Domain)
        ...
        call cpu_clock_begin(id_clock_diabatic)
        call adiabatic(h, CS%tv, fluxes, CS%dt_trans, CS%visc%MLD, G, GV, CS%diabatic_CSp)
        fluxes%fluxes_used = .true.
        call cpu_clock_end(id_clock_diabatic)
...
subroutine step_tracers(fluxes, state, Time_start, time_interval, CS)
    ...
    if(CS%use_ALE_algorithm) then
      ...
      call call_tracer_column_fns(h_pre, h_new, eatr*0.5, ebtr*0.5, &
              fluxes, CS%offline_CSp%dt_offline*0.5, CS%visc%MLD, G, GV, CS%tv, &
      ...
      do iter=1,CS%offline_CSp%num_off_iter
        ...
        if (z_first) then
          ...
          call call_tracer_column_fns(h_pre, h_new, eatr_sub, ebtr_sub, &
              fluxes, dt_iter, CS%visc%MLD, G, GV, CS%tv, CS%diabatic_CSp%optics, CS%tracer_flow_CSp, CS%debug)
