...
contains
  ...
  subroutine da_const_inc_init(CS, G, GV, US, param_file, h, T, S, eqn_of_state)
   ...
   subroutine send_prior_recv_increment(CS, G, GV, dt, h, T, S, reset_state)
     type(da_const_inc_type), intent(inout) :: CS !< Control structure for data assimilation
     type(ocean_grid_type),   intent(in   ) :: G  !< Ocean grid structure
     type(verticalGrid_type), intent(in   ) :: GV !< ocean vertical grid structure
     ...
     logical,                 intent(  out) :: reset_state !< If true, the prior step has been completed and
                                                           !! the posterior integration should be ready
     ...
     if (CS%do_temp) then
       ...
       call diag_remap_do_remap(CS%diag_remap, G, GV, h, .false., .false., CS%obs_mask, 0., T, CS%temp_prior)
     ...
     if (CS%do_salt) then
       ! Set status key for send prior to not done
       ! Set status key for receive increment to not ready
       call diag_remap_do_remap(CS%diag_remap, G, GV, h, .false., .false., CS%obs_mask, 0., S, CS%salt_prior)
